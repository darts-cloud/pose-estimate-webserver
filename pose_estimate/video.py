...
class VideoCapture:
    ...
    def __init__(self, src, dest=None, fps=None, width=None, height=None):
        try:
            ...
            self.src = str(src)
        logger.info(f"Connect: Camera:{self.src!s}")
        ...
        logger.info(
            f"Video Param: FPS={int(self.fps)}, Width={int(self.width)}, Height={int(self.height)}"
        )
    ...
    def output_setting(self, output_path):
        ...
        self.out = cv2.VideoWriter(
            path, fourcc, self.fps, (int(self.width), int(self.height))
        )  # 出力先のファイルを開く
...
class ThreadingVideoCapture(VideoCapture):
    ...
    def output_setting(self, output_path):
        ...
        fourcc = cv2.VideoWriter_fourcc("m", "p", "4", "v")
        self.out = cv2.VideoWriter(
            output_path, fourcc, frame_rate, (int(width), int(height))
        )  # 出力先のファイルを開く
