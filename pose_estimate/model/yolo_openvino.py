...
class YoloModel(): 
    ...
    def __init__(self, pose_model_path, size, imgsz=None, threshold=0.85):
        ...
        model_path = "model\\yolo11x-pose_int8_openvino_model\\"
        if True:
        ...
        core = ov.Core()
        mdl = core.read_model(model=model_path + "yolo11x-pose-int8.xml")
                
        # CPU用にコンパイル
        self._model = core.compile_model(model=mdl,device_name="GPU")        # self._model = YOLO(model_path)
